...
function generateHtmlReport(testData, summaryData, outputPath) {
  ...
  const detailRowsHtml = testData.map((t, idx) => `
    <tr class="test-row ${t.status.toLowerCase()}" data-category="${t.category.replace(/"/g, '&quot;')}">
      ...
      <td>${t.error ? `<details><summary class="err-summary">View Error</summary><pre class="err-stack">${t.error}</pre></details>` : '<span class="text-muted">-</span>'}</td>
    </tr>
  `).join('');
...
<head>
  ...
  <title>Smart Finance AI — 1,100 E2E Selenium Test Report</title>
  <style>
    :root {
      --bg-dark: #0f172a;
      --card-bg: #1e293b;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .err-stack { background: #090d16; padding: 10px; border-radius: 6px; color: #fca5a5; margin-top: 6px; white-space: pre-wrap; font-size: 0.75rem; }
...
<body>
  ...
  <header>
    <div>
      <h1>Smart Finance AI — E2E Test Execution Report</h1>
      <div class="subtitle">1,100 Selenium Web Assertions across 110 Test Categories</div>
    </div>
  ...
  <h2 class="section-title">Testing Types Aggregated Summary (110 Categories)</h2>
  <table>
    <thead>
      <tr>
        <th>#</th>
  ...
  <div class="section-title">
    <span>Detailed Test Results (1,100 Cases)</span>
